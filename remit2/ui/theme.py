...
.r2-mast h1 {{ font-size:1.55rem; font-weight:800; color:var(--r2-ink); margin:0; letter-spacing:-.01em; }}
...
.r2-tile .label {{ font-size:.85rem; font-weight:700; letter-spacing:.04em; text-transform:uppercase; color:var(--r2-ink-soft); width:100%; }}
.r2-tile .value {{ font-size:2.1rem; font-weight:800; line-height:1; color:var(--r2-ink); font-variant-numeric:tabular-nums; }}
.r2-tile .unit {{ font-size:1rem; font-weight:600; color:var(--r2-ink-soft); margin-left:.25rem; }}
.r2-tile .sub {{ font-size:.92rem; color:var(--r2-ink-soft); }}
.r2-meter {{ width:100%; height:9px; background:#e5e7eb; border-radius:6px; overflow:hidden; margin-top:.35rem; }}
.r2-meter > span {{ display:block; height:100%; background:var(--r2-ink); border-radius:6px; }}
.r2-next {{ font-size:.95rem; color:var(--r2-ink); margin-top:.4rem; }}
.r2-chips {{ display:flex; flex-wrap:wrap; gap:.35rem; margin-top:.45rem; }}
.r2-chip {{ display:inline-flex; align-items:center; gap:.35rem; font-size:.8rem; font-weight:700;
...
.r2-strip b {{ font-weight:800; }}
.r2-strip .stat {{ padding:.15rem .55rem; border-radius:8px; background:#f1f5f9; }}
/* Section header */
.r2-sec {{ font-size:1.05rem; font-weight:800; color:var(--r2-ink); margin:.4rem 0 .2rem; }}
...
body.r2-wall .r2-tile .value {{ font-size:2.8rem; }}
...
.r2-tile .value { font-size:2.8rem; }
